...
ft_write:
        mov rax, 1                     ; Return value of write syscall is 1
        syscall                        ; Syscall entrypoint is in rax, we gave 1 so it calls write
        cmp rax, 0                     ; Is write return is null ?
        jl end_err                     ; jl (if lower) : if the destination operand is less than the source operand, jump to end_err
...
end_err:
        ...
        mov [rax], rdx                 ; Putting rdx (we set before) in rax value, syscall error is stored in errno
